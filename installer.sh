...
##############################################################################################
## This script is to install Kubernetes On Premise. Script must be run on a fresh installed ## 
## Ubuntu (v 18.04 LTS), Debian 10, or CentOS 7 Linux.                                      ##
##############################################################################################

NODE="$1"

if [ ! "$NODE" == "master" ] && [ ! "$NODE" == "worker" ] && [ ! "$NODE" == "slb" ] && [ ! "$NODE" == "database" ] && [ ! "$NODE" == "dns" ] && [ ! "$NODE" == "nfs" ] && [ ! "$NODE" == "minio" ]; then
    echo "First parameter is node type. Should be filled by either 'master', 'worker', 'slb', 'database', 'dns', 'nfs' and 'minio'."
    exit 1
fi

if [ "$NODE" == "master" ]; then
    echo "Welcome! You will now install Kubernetes master node. By the end of the successful installation, you will get these kind of data: "
    echo "  - Master node's IP Address (and port)"
    echo "  - A token"
    echo "  - A hashed CA Certificate for discovery token"
    echo "Please keep these information as you will need them to install worker nodes."
    echo "Please type 'continue' to continue installation process: "
    read RESPONSE
...
if [ "$NODE" == "worker" ]; then
    echo "Welcome! You will now install worker node. Before install worker node, you will need to install master node and I will ask you to enter several information provided by master node."
    echo "To continue, please type 'continue' below: "
    read RESPONSE
    if [ ! "$RESPONSE" == "continue" ]; then
        exit 1
    fi
...
if [ "$NODE" == "slb" ]; then
    echo "Welcome! You will install HaProxy Server Load Balancer. Before install SLB, you will need to have the read service to be load balanced and a domain for that service. I will ask you about this information after this."
    echo "To continue, please type 'continue' below: "
    read RESPONSE
    if [ ! "$RESPONSE" == "continue" ]; then
        exit 1
...
if [ "$NODE" == "database" ]; then
    echo "Install database."
    echo "Welcome! You will install PostgreSQL 10 database server. I will ask later the username you choose for the database. The password for that user will be generated randomly which I will tell you later."
    echo "To continue, please type 'continue' below: "
    read RESPONSE
    if [ ! "$RESPONSE" == "continue" ]; then
        exit 1
    fi
    echo "Please enter username you choose for database: "
    read DB_USERNAME
    echo "(echo) Username: $DB_USERNAME"
fi

if [ "$NODE" == "dns" ]; then
    echo "Welcome! You will install Bind9 for Domain Name Server. Before the installation started, make sure you have installed SLB for both frontend and backend. I will ask the IP addresses and domain you choose for that SLB."
    echo "To continue, please type 'continue' below: "
    read RESPONSE
    if [ ! "$RESPONSE" == "continue" ]; then
        exit 1
...
if [ "$NODE" == "nfs" ]; then
    echo "Welcome! You will now install Network File Server. This server will be used as the storage server for your files (configurations, uploaded files and so on). The files uploaded will be stored in /mnt/shared folder."
    echo "To continue, please type 'continue' below: "
    read RESPONSE
    if [ ! "$RESPONSE" == "continue" ]; then
        exit 1
    fi
    echo "Please enter Worker's Subnet IP Address (e.g. 192.168.1.0/24): "
    read WORKER_SUBNET_IP
    echo "(echo) Worker's Subnet IP: $WORKER_SUBNET_IP"
fi

if [ "$NODE" == "minio" ]; then
    echo "Welcome! You will now install Minio. This server will be used as interface to store your file (similar to AWS S3 or Alicloud OSS)."
    echo "To continue, please type 'continue' below: "
    read RESPONSE
    if [ ! "$RESPONSE" == "continue" ]; then
        exit 1
    fi
    echo "Please enter Minio Access Key: "
    read MINIO_ACCESS_KEY
    echo "(echo) Minio's Access Key: $MINIO_ACCESS_KEY"
    echo "Please enter Minio Secret Key: "
    read MINIO_SECRET_KEY
    echo "(echo) Minio's Secret Key: $MINIO_SECRET_KEY"
    echo "Please enter NAS IP Address (e.g. 192.168.0.11): "
    read NAS_IP_ADDRESS
    echo "(echo) NAS IP Address: $NAS_IP_ADDRESS"
    echo "Please enter NAS exported folder (e.g. /mnt/shared): "
    read NAS_EXPORTED_FOLDER
    echo "(echo) NAS exported folder: $NAS_EXPORTED_FOLDER"
fi

## Step 1. Check the linux version
OS_TYPE="$( get_os )"
if [ "$OS_TYPE" == "linux" ]; then
    DISTRO="$( get_linux_distro )"
    if [ "$DISTRO" == "Ubuntu" ] || [ "$DISTRO" == "CentOS" ] || [ "$DISTRO" == "Debian" ]; then
        DISTRO_VERSION="$( get_linux_distro_version $DISTRO )"
